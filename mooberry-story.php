...
function mbds_init() {
	...
	register_taxonomy( 'mbds_genre', 'mbds_story',
		apply_filters( 'mdbs_genre_taxonomy', array(
			//'rewrite' => false,
			'rewrite'               => array( 'slug' => 'mbds_genres', 'with_front' => false ),  //jmihalik customization to remove /blog url
			'public'                => true, //false,
	...
	register_taxonomy( 'mbds_series', 'mbds_story',
		apply_filters( 'mbds_series_taxonomy', array(
			'rewrite'               => array( 'slug' => 'mbds_series', 'with_front' => false ),  //jmihalik customization to remove /blog url
			'public'                => true, // false,
			'show_admin_column'     => true,
			'update_count_callback' => '_update_post_term_count',
			'capabilities'          => array(
...
function mbds_content( $content ) {
	...
	if ( ! is_single() ) {
		...
		} else if ( get_post_type() == 'mbds_story' && is_main_query() && ! is_admin() ) {
			// If it's not a single post AND a story post, then this is probably a custom taxonomy page. Add some info to make it a little prettier.
